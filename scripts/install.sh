#!/usr/bin/env bash
set -Eeuo pipefail
APP_DIR=${APP_DIR:-/opt/chatgpt2api}; PORT=${PORT:-4000}; REPO_URL=${REPO_URL:-https://github.com/qingan123/chatgpt2api.git}
...
APP_DIR=$(read_tty "目录 [$APP_DIR]: "); APP_DIR=${APP_DIR:-/opt/chatgpt2api}; PORT=$(read_tty "端口 [$PORT]: "); PORT=${PORT:-4000}; [[ $PORT =~ ^[0-9]+$ ]] || fail '端口无效'; ss -ltn "sport = :$PORT" 2>/dev/null | grep -q LISTEN && fail "端口 $PORT 已被占用"
